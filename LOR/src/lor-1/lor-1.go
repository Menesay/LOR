...
var (
	// lor-0 will connect
	// 1) LOR1IP:cliDATAPORT 			to send host:port data
	// 2) LOR1IP:cliPROXYPORT			to get proxy data
	cliDATAPORT  string
	cliPROXYPORT string

	// lor-1 will connect
	// 1) serLOR2IP:serLOR2DATAPORT 	to send host:port data
	// 2) serLOR2IP:serLOR2PROXYPORT	to get proxy data
	serLOR2IP        string
	serLOR2DATAPORT  string
	serLOR2PROXYPORT string
...
func handleArgs() {
	...
	if os.Args[1] == "-d" {
		...
		serLOR2IP = os.Args[3]
		serLOR2DATAPORT = os.Args[4]
		serLOR2PROXYPORT = os.Args[5]
...
func help() {
	...
	helpMsg := ""
	helpMsg += "Useage:\n"
	helpMsg += "lor-1 cliDATAPORT cliPROXYPORT serLOR2IP serLOR2DATAPORT serLOR2PROXYPORT\n"
	helpMsg += "lor-1 -d 	debug mode (optional)"
	fmt.Println(helpMsg)
...
func main() {
	...
	// Debug
	fmt.Println(cliDATAPORT, cliPROXYPORT, serLOR2IP, serLOR2DATAPORT, serLOR2PROXYPORT)
